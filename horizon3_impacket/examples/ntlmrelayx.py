...
from horizon3_impacket.impacket.examples.ntlmrelayx.servers import SMBRelayServer, HTTPRelayServer, WCFRelayServer
from horizon3_impacket.impacket.examples.ntlmrelayx.utils.config import NTLMRelayxConfig
from horizon3_impacket.impacket.examples.ntlmrelayx.utils.targetsutils import TargetsProcessor, TargetsFileWatcher
from horizon3_impacket.impacket.examples.ntlmrelayx.servers.socksserver import SOCKS
...
def start_servers(options, threads, tool_q=None):
...
def main(raw_args=None, tool_q=None):
    ...
    parser.add_argument('-w', '--watch', action='store_true', help='Watch the target file for changes and update target list '
                                                        'automatically (only valid with -tf)', default=True)
    parser.add_argument('--interactive', action='store_true',help='Launch an smbclient or LDAP console instead'
    ...
    if options.debug is True:
        ...
        logging.getLogger('horizon3_impacket.impacket.smbserver').setLevel(logging.ERROR)

    # Let's register the protocol clients we have
    # ToDo: Do this better somehow
    from horizon3_impacket.impacket.examples.ntlmrelayx.clients import PROTOCOL_CLIENTS
    from horizon3_impacket.impacket.examples.ntlmrelayx.attacks import PROTOCOL_ATTACKS
    global socksServer
    global targetSystem
    global codec
    global mode
    global PROTOCOL_CLIENTS
    global PROTOCOL_ATTACKS
    ...
    if targetSystem is not None and options.watch:
    ...
    c = start_servers(options, threads, tool_q=tool_q)
