...
def ParseSQLHash(data, client, Challenge, tool_q=None):
    SSPIStart = data[8:]
    ...
    if NthashLen == 24:
        WriteHash = '%s::%s:%s:%s:%s' % (
        User, Domain, LMHash, NTHash, codecs.encode(Challenge, 'hex').decode('latin-1'))
        if tool_q != None:
    ...
    if NthashLen > 60:
        WriteHash = '%s::%s:%s:%s:%s' % (
        User, Domain, codecs.encode(Challenge, 'hex').decode('latin-1'), NTHash[:32], NTHash[32:])
...
def ParseClearTextSQLPass(data, client, tool_q=None):
            ...
            while True:
                ...
                if data[0] == b"\x10" or data[0] == 16:  # NegoSSP
                    if re.search(b'NTLMSSP', data):
                        ...
                        ParseClearTextSQLPass(data, self.client_address[0], tool_q=self.server.tool_q)

                if data[0] == b'\x11' or data[0] == 17:  # NegoSSP Auth
                    ParseSQLHash(data, self.client_address[0], Challenge, tool_q=self.server.tool_q)
