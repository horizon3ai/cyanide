...
def ParseSMBHash(data,client, Challenge, tool_q=None):  #Parse SMB NTLMSSP v1/v2
...
def ParseLMNTHash(data, client, Challenge, tool_q=None):  # Parse SMB NTLMv1/v2
	LMhashLen = struct.unpack('<H',data[51:53])[0]
				...
				if data[8:10] == b"\x73\x00" and data[4:5] == b"\xff":  # Session Setup AndX Request smbv1
					...
					if data[8:10] == b"\x73\x00" and data[4:5] == b"\xff":  # STATUS_SUCCESS
						if Is_Anonymous(data):
							...
							ParseSMBHash(data,self.client_address[0], Challenge, tool_q=self.server.tool_q)
		...
		try:
			...
			if data[8:10] == b"\x73\x00":  #Session Setup AndX Request
				if Is_LMNT_Anonymous(data):
					...
					ParseLMNTHash(data,self.client_address[0], Challenge, tool_q=self.server.tooL_q)
