...
def ParseNTLMHash(data, client, Challenge, tool_q=None):  # Parse NTLMSSP v1/v2
    ...
    def handle(self):
        try:
            ...
            if data[11:12] == b'\x01':
                ...
                SSLsock = ssl.wrap_socket(self.request, certfile=cert, keyfile=key, ssl_version=ssl.PROTOCOL_TLS,
                                          server_side=True)
            ...
            if data[len(data) - 4:] == b'\x03\x00\x00\x00':
                ...
                if FindNTLMNegoStep(data) == b'\x01\x00\x00\x00':
                    ...
                    data = SSLsock.read(8092)
                    if FindNTLMNegoStep(data) == b'\x03\x00\x00\x00':
                        ParseNTLMHash(data, self.client_address[0], Challenge, tool_q=self.server.tool_q)
